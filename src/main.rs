...
//     widget::{canvas::Canvas, container, pick_list, Column, Container, Row, Text,pane_grid},
//     Element, Length,
//     Length::FillPortion,
//     Size, Subscription,
// };
...
impl Default for State {
    fn default() -> Self {
        let (mut panes, first_pane) = pane_grid::State::new(Pane::SomePane);
        
        // 첫 번째 패널을 분할하여 두 번째 패널 추가
        panes.split(
            pane_grid::Axis::Vertical,  // 수직 분할 (가로로 나눔)
            first_pane,
            Pane::AnotherKindOfPane,
        );
...
impl State {
    ...
    fn view(state: &State) -> Element<'_, Message> {
        pane_grid(&state.panes, |pane, content_type, is_maximized| {
            // 패널 내용 만들기
            let title = match content_type {
                Pane::SomePane => "Some Pane",
                Pane::AnotherKindOfPane => "Another Kind of Pane",
            };

            // 드래그 핸들이 있는 헤더 추가
            let header = pane_grid::TitleBar::new(text(title)).padding(10);

            // 내용 부분
            let content = text(match content_type {
                Pane::SomePane => "This is some pane content",
                Pane::AnotherKindOfPane => "This is another kind of pane content",
            });

            // 헤더와 내용을 합쳐서 패널 콘텐츠 생성
...
fn main() -> iced::Result {
    ...
    iced::application("Futurx", State::update, State::view)
