...
pub fn main() -> iced::Result {
    SimpleChart::run(Settings::default())
}
...
impl Application for SimpleChart {
    ...
    fn new(_flags: ()) -> (Self, Command<Message>) {
        ...
        (SimpleChart { data }, Command::none())
    }

    fn title(&self) -> String {
        String::from("Simple Chart")
    }
...
impl<Message> Program<Message> for  ChartWidget {
    fn draw(&self, bounds: Rectangle, _cursor: canvas::Cursor) -> Vec<canvas::Geometry> {
        ...
        let points: Vec<Point> = self.data
            ...
            .map(|(i, &value)| {
                Point::new(
                    (i as f32 / (self.data.len() - 1) as f32) * bounds.width,
                    bounds.height - (value / max_value * bounds.height),
                )
            })
            .collect();
